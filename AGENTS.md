...
Keep preprocessing, annotation, dataset generation, evaluation, and training concerns separable. Shared logic should have one canonical implementation rather than being duplicated between scripts and the UI.

All model annotation windows must be derived deterministically from the original unitized book. PAST and FUTURE are context only; every source unit belongs to exactly one TARGET region for a given windowing configuration.

Concurrent requests must not depend on predictions from earlier windows. Responses may arrive out of order and must be merged by stable book/model/method/window identity.

Use error-aware retry behavior. Do not treat authentication errors, rate limits, transient server failures, malformed model outputs, and exhausted quotas as equivalent failures.

Validate structured model output before accepting it. A successful HTTP response is not automatically a valid annotation.

Prefer explicit schemas and typed boundaries between components. Validate external inputs and persisted research artifacts rather than assuming shape correctness.

## 5. Testing Expectations

Add tests for behavior that can invalidate research results, especially:

- deterministic cleaning and normalization
- stable unit-ID generation
- paragraph and heading preservation
- scene-separator removal
- PAST / TARGET / FUTURE ownership
- no duplicate TARGET ownership
- token-budget enforcement using the exact serialized training format
- annotation JSON validation
- out-of-order concurrent result merging
- retry/key failure handling
- hash/version mismatch rejection
- reconstruction reproducibility

For deterministic components, prefer exact expected-output tests over vague smoke tests.

Do not weaken or delete a failing research-integrity test merely to make CI pass. If the specification itself is wrong, surface that as a research decision.

## 6. Working With Codex

Before implementing a substantial feature, inspect the relevant repository specifications and existing tests.

When a task is underspecified, distinguish between an ordinary engineering detail and a research-design decision. Reasonable engineering details may be implemented directly; research-design decisions covered by the constraints above require explicit user approval.

When completing work, summarize what changed, what invariants are now enforced, what tests were added or run, and any unresolved research decisions. Do not present an implementation choice as an experimental fact unless it is documented in the repository's research specifications.
